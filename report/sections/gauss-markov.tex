...
được Tạo ra như thế nào. Tuy nhiên, để nói về \textbf{chất lượng thống kê} của ước
lượng — tính không chệch, phương sai, và tính tối ưu — ta cần một số giả thiết về cách dữ liệu được Tạo
...
GM1 phát biểu mô hình tuyến tính theo tham số đã mô tả đúng cấu trúc kỳ vọng có
điều kiện của dữ liệu. Không thể kết luận GM1 thỏa mãn chỉ bằng cách gán một cờ
\texttt{True}, và cũng không thể chứng minh tuyệt đối giả thiết này chỉ từ một
mẫu quan sát. Tuy nhiên, đồ thị \textbf{Residuals vs Fitted} có thể phát hiện
dấu hiệu mô hình tuyến tính bị sai dạng: nếu phần dư tạo thành đường cong hoặc
xu hướng có hệ thống theo fitted values, mô hình có thể đang thiếu thành phần
phi tuyến hoặc interaction quan trọng.

Hàm \texttt{verify\_assumptions} vì vậy trả
\texttt{GM1\_Linearity=None}, kèm fitted values, residuals và trung bình phần dư
theo từng bin để phục vụ chẩn đoán. Hình~\ref{fig:gm1_linearity_diagnostic}
minh họa kết quả trên dữ liệu mô phỏng tuyến tính. Các điểm xanh là phần dư từng
quan sát; đường đỏ nối trung bình phần dư trong các khoảng fitted.
...
Đường trung bình phần dư dao động quanh đường 0 và không tạo thành dạng cong có
hệ thống, do đó \textbf{không thấy dấu hiệu phi tuyến rõ rệt} trong mẫu này.
Kết luận đúng phải được diễn đạt ở mức chẩn đoán như vậy, thay vì khẳng định
``GM1 đã thỏa mãn''. Nếu đường đỏ có dạng chữ U, chữ S hoặc xu hướng tăng/giảm
rõ rệt, cần bổ sung biến phi tuyến, interaction hoặc thay đổi dạng hàm.
...
\textbf{phương sai nhỏ nhất} trong Class các ước lượng tuyến tính không chệch. Đây là các
...
\begin{itemize}[noitemsep, leftmargin=1.4cm]
    ...
    \item Ma trận $\mat{X}$: cột 1 toàn số 1, hai cột còn lại Tạo từ $\mathcal{N}(0,1)$,
          \textbf{cố định} suốt mô phỏng.
    \item Ước lượng đối chứng Alt: $\mat{A} = \mat{B}(\mat{I}-\mat{H})$ với $\mat{B}$ là
          ma trận hằng cố định (Tạo ngẫu nhiên một lần, $\mathtt{alt\_scale} = 0.06$).
\end{itemize}

\subsubsubsection{Cài đặt Python}

Ước lượng OLS tái sử dụng hàm \texttt{ols\_fit} đã xây từ đầu. Ước lượng đối chứng Alt và
vòng lặp Monte Carlo được lưu trong file
\texttt{part1/monte\_carlo\_gauss\_markov.py}. Cài đặt
sử dụng NumPy chỉ để Tạo nhiễu, tổng hợp kết quả và kiểm chứng; các phép toán ma trận được
xây dựng từ đầu.
