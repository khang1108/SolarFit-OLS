\subsection{Định Lý Gauss--Markov và Kiểm Chứng Monte Carlo}

\subsubsection{Các Giả Thiết Gauss--Markov}

\subsubsubsection{Bối cảnh}

Xét về bản chất thuần túy đại số, công thức nghiệm $\hat{\boldsymbol{\beta}} = (\mat{X}^T\mat{X})^{-1}\mat{X}^T\mat{y}$ tối thiểu hóa tổng bình phương phần dư một cách cơ học bất kể cơ chế sinh dữ liệu thực sự là gì; bởi vậy nghiệm này luôn tồn tại và được xác định hoàn toàn từ $\mat{X}$ và $\mat{y}$ mà không cần bất kỳ giả thiết xác suất nào. Tuy nhiên, để có thể đánh giá \textbf{chất lượng thống kê} của ước lượng, tức xét xem $\hat{\boldsymbol{\beta}}$ có thực sự gần $\boldsymbol{\beta}$ thật không, phân tán ra sao qua các mẫu lặp lại, và liệu có ước lượng tuyến tính nào khác hiệu quả hơn không, ta cần đặt ra một số giả thiết về quá trình sinh dữ liệu. Những giả thiết này, được gọi chung là \textbf{giả thiết Gauss--Markov}, xác định khuôn khổ lý thuyết để chứng minh OLS là ước lượng tốt nhất trong lớp các ước lượng tuyến tính không Bias.
...
\begin{description}[leftmargin=1.4cm, style=nextline, font=\bfseries\color{primaryblue}]
    ...
    \item[GM3.] \textbf{Ngoại Tạo nghiêm ngặt (kỳ vọng nhiễu bằng 0):}
...
đảm nghiệm OLS tồn tại và duy nhất, GM3 cho tính không Bias, còn GM4 cho tính tối ưu
...
\subsubsubsection{Chẩn đoán trực quan giả thiết GM1}

Giả thiết GM1 phát biểu rằng kỳ vọng có điều kiện $\E[y_i \mid \mat{x}_i] = \mat{x}_i^T\boldsymbol{\beta}$ mô tả đúng cấu trúc của dữ liệu. Vì đây là một giả thiết về dạng hàm của quá trình sinh dữ liệu, về mặt phương pháp luận thống kê không có kiểm định nào có thể xác nhận tuyệt đối GM1 chỉ từ một mẫu hữu hạn; thay vào đó, điều ta có thể làm là quan sát xem phần dư có bộc lộ dấu hiệu phi tuyến còn sót lại hay không. Đồ thị \textbf{Residuals vs Fitted} là công cụ chẩn đoán trực quan chuẩn cho mục đích này: nếu phần dư tạo thành đường cong hoặc xu hướng có hệ thống theo chiều tăng của fitted values, điều đó gợi ý mô hình đang thiếu thành phần phi tuyến hoặc interaction quan trọng; ngược lại, khi các điểm phân tán đều quanh đường 0 mà không tạo hình dạng rõ rệt, đó là dấu hiệu tích cực cho thấy GM1 không bị vi phạm nghiêm trọng.

Hình~\ref{fig:gm1_linearity_diagnostic} trình bày kết quả chẩn đoán trên dữ liệu mô phỏng tuyến tính, trong đó các điểm xanh là phần dư từng quan sát và đường đỏ nối trung bình phần dư trong từng dải fitted values.

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{images/part1_gm1_linearity_diagnostic.png}
\caption{Chẩn đoán GM1 bằng Residuals vs Fitted và trung bình phần dư theo bin.}
\label{fig:gm1_linearity_diagnostic}
\end{figure}

\noindent\textbf{Nhận xét.}
Đường trung bình phần dư dao động đều quanh số 0 theo toàn dải fitted values mà không tạo thành dạng cong hay xu hướng có hệ thống nào, do đó kết luận chẩn đoán là \textbf{không thấy dấu hiệu phi tuyến rõ rệt} trong mẫu được kiểm tra. Cần lưu ý rằng kết luận đúng đắn nên được giữ ở mức ``không thấy bằng chứng vi phạm GM1'' thay vì ``GM1 đã được chứng minh thỏa mãn'', vì bất kỳ khẳng định tuyệt đối nào cũng vượt quá giới hạn của chẩn đoán dựa trên mẫu hữu hạn. Trong trường hợp đường đỏ có dạng chữ U, chữ S hoặc xu hướng đơn điệu rõ rệt, mô hình cần được bổ sung biến phi tuyến, interaction hoặc điều chỉnh dạng hàm cho phù hợp hơn với cấu trúc thực của dữ liệu.

% ============================================================
\subsubsection{Định Lý Gauss--Markov (BLUE)}

\subsubsubsection{Phát biểu}
...
là \textbf{ước lượng tuyến tính không Bias tốt nhất} (\textit{Best Linear Unbiased
Estimator} --- BLUE):
\begin{enumerate}[label=(\roman*), noitemsep]
    \item \textbf{Tuyến tính:} $\hat{\boldsymbol{\beta}}_{\mathrm{OLS}}$ là một hàm
          tuyến tính của $\mat{y}$.
    \item \textbf{Không Bias:} $\E[\hat{\boldsymbol{\beta}}_{\mathrm{OLS}}\mid\mat{X}]
          = \boldsymbol{\beta}$.
    \item \textbf{Tốt nhất:} Với mọi ước lượng tuyến tính không Bias khác
...
Xét một ước lượng tuyến tính không Bias bất kỳ
...
\emph{Ràng buộc từ tính không Bias.} Tương tự (ii),
...
$\hat{\boldsymbol{\beta}}_{\mathrm{OLS}}$ có phương sai nhỏ nhất trong Class các ước lượng
tuyến tính không Bias, tức là BLUE.
...
% ============================================================
\subsubsection{Minh Họa Định Lý Gauss--Markov bằng Mô Phỏng Monte Carlo}

\subsubsubsection{Ý tưởng}

Định lý Gauss--Markov khẳng định hai tính chất của $\hat{\boldsymbol{\beta}}_{\mathrm{OLS}}$:
\textbf{không Bias} ($\E[\hat{\boldsymbol{\beta}}\mid\mat{X}] = \boldsymbol{\beta}$) và
\textbf{phương sai nhỏ nhất} trong Class các ước lượng tuyến tính không Bias. Đây là các
mệnh đề về \textit{kỳ vọng} và \textit{phương sai}, vốn là những đại lượng không quan sát được
từ một mẫu duy nhất. Mô phỏng Monte Carlo cho phép \textbf{xấp xỉ} chúng: cố định ma trận
thiết kế $\mat{X}$ và tham số thật $\boldsymbol{\beta}$, ta tạo lặp lại nhiều bộ dữ liệu
chỉ khác nhau ở nhiễu ngẫu nhiên, rồi quan sát phân phối của các ước lượng thu được.

\subsubsubsection{Công thức ước lượng Monte Carlo}

Với mỗi lần lặp $r = 1, \ldots, M$, ta giữ nguyên $\mat{X}$ và tạo
\[
...
Để minh họa tính ``tốt nhất'', ta cần một ước lượng tuyến tính không Bias \textit{khác}
...
\begin{equation}
    ...
    \label{eq:alt}
\end{equation}
Điều kiện $\mat{A}\mat{X} = \mat{0}$ làm cho Alt vẫn \textbf{không Bias}:
...
OLS, phù hợp với kết luận của định lý.
...
thay đổi. Nếu tạo lại $\mat{X}$ mỗi lần, ta sẽ trộn lẫn biến thiên của thiết kế vào kết quả.
\end{note}

\subsubsubsection{Thiết lập thực nghiệm}

\begin{table}[H]
\centering
\caption{Thiết lập thực nghiệm Monte Carlo.}
\label{tab:mc_setup}
\begin{tabular}{>{\raggedright}p{0.30\textwidth}
                >{\raggedright\arraybackslash}p{0.62\textwidth}}
\toprule
\textbf{Tham số} & \textbf{Giá trị} \\
\midrule
Số quan sát $n$; số tham số $p+1$
    & $n = 30$;\quad $p+1 = 3$;\quad $\boldsymbol{\beta} = (2.0,\; 1.0,\; -0.5)^T$ \\[4pt]
Phương sai nhiễu; số lần lặp
    & $\sigma^2 = 1.0$;\quad $M = 8000$ \\[4pt]
Ma trận thiết kế $\mat{X}$
    & Cột 1 toàn số 1; cột 2--3 sinh từ $\mathcal{N}(0,1)$, \textbf{cố định} suốt toàn bộ mô phỏng \\[4pt]
Ước lượng đối chứng Alt
    & $\mat{A} = \mat{B}(\mat{I}-\mat{H})$, $\mat{B}$ là ma trận hằng tạo ngẫu nhiên một lần ($\mathtt{alt\_scale} = 0.06$) \\
\bottomrule
\end{tabular}
\end{table}

\subsubsubsection{Cài đặt Python}

Toàn bộ mô phỏng được tổ chức trong file \texttt{part1/monte\_carlo\_gauss\_markov.py}, trong đó ước lượng OLS tái sử dụng trực tiếp hàm \texttt{ols\_fit} đã xây dựng từ đầu, còn vòng lặp $M$ lần lặp và ước lượng đối chứng Alt đều được cài đặt độc lập mà không phụ thuộc vào thư viện tối ưu ngoài. NumPy chỉ được dùng cho ba tác vụ hạn chế là sinh nhiễu ngẫu nhiên $\boldsymbol{\varepsilon}^{(r)}$, tính trung bình và hiệp phương sai mẫu kết quả, và kiểm chứng nghiệm so với oracle, đúng theo nguyên tắc scratch-first của toàn bộ project.

\subsubsubsection{Kết quả mô phỏng}
...
xác nhận \textbf{tính không Bias}. Cột phương sai cho thấy Alt lớn hơn OLS ở \textit{mọi}
hệ số (tỉ số $> 1$), qua đó minh họa trực tiếp \textbf{tính tối ưu} của OLS theo nghĩa BLUE.

\begin{table}[H]
\centering
\caption{Trung bình và phương sai sau $M = 8000$ mô phỏng (OLS vs Alt).
Độ Bias lớn nhất: $\max|\text{bias}_{\mathrm{OLS}}| = 4.9\times10^{-4}$,
$\max|\text{bias}_{\mathrm{Alt}}| = 4.6\times10^{-3}$; cả hai không Bias, nhưng phương
...
cũng cho thấy độ Bias của \textit{cả} OLS lẫn Alt đều $\le 4.6\times10^{-3}$, nhỏ hơn
nhiều so với độ phân tán của ước lượng. (Độ Bias thực nghiệm của Alt hơi lớn hơn OLS chỉ
...
\noindent\textbf{Nhận xét.} Cả hai phân phối đều \textbf{cùng tâm} tại giá trị thật (vạch đỏ), xác nhận rằng OLS và Alt đều không Bias. Tuy nhiên, phân phối OLS (xanh) \textbf{cao và hẹp hơn}, tức tập trung quanh $\boldsymbol{\beta}$ chặt hơn, trong khi Alt (cam) trải rộng ở cả ba hệ số; đây là minh họa trực quan cho tính chất phương sai nhỏ nhất của OLS trong lớp các ước lượng tuyến tính không Bias (BLUE).
...
gần như bằng nhau, cho thấy mô phỏng khớp chặt với công thức $\sigma^2(\mat{X}^T\mat{X})^{-1}$
\eqref{eq:ols_cov} và $\mathrm{Var}(\hat{\boldsymbol{\beta}}_{\mathrm{OLS}}) + \sigma^2\mat{A}\mat{A}^T$
\eqref{eq:alt_var}. Cột Alt cao hơn cột OLS ở \textbf{mọi} hệ số với tỉ số từ $2.5$ đến $4.5$,
phù hợp với kết luận BLUE của định lý.

\subsubsubsection{Kiểm chứng với NumPy và scikit-learn}
...
Hàm \texttt{verify\_with\_numpy\_sklearn()} trong
\texttt{part1/monte\_carlo\_gauss\_markov.py}
thực hiện kiểm chứng này:

Kết quả cho thấy sai số cực đại so với cả hai oracle đều đạt độ chính xác máy:
$\norm{\hat{\boldsymbol{\beta}}_{\text{tự xây dựng}} - \hat{\boldsymbol{\beta}}_{\text{NumPy}}}_\infty
= \norm{\hat{\boldsymbol{\beta}}_{\text{tự xây dựng}} - \hat{\boldsymbol{\beta}}_{\text{sklearn}}}_\infty
= 6.66\times10^{-16}$.
Ngoài ra, phương sai thực nghiệm Monte Carlo khớp với công thức lý thuyết ở mọi hệ số
(xem chi tiết tại Bảng~\ref{tab:mc_results}).

\begin{note}[Kết luận]
Mô phỏng Monte Carlo xác nhận đầy đủ Định lý Gauss--Markov: ước lượng OLS \textbf{không
Bias} (trung bình trùng giá trị thật) và có \textbf{phương sai nhỏ nhất} so với ước lượng
tuyến tính không Bias đối chứng (Alt). Cài đặt từ đầu trùng khớp NumPy/scikit-learn tới
